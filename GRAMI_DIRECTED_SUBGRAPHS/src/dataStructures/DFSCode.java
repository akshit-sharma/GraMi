 ...
 *            .equals(..))
 */
public class DFSCode<NodeType, EdgeType>
    extends SearchLatticeNode<NodeType, EdgeType>
    implements Comparable<DFSCode<NodeType, EdgeType>>,
               Generic<NodeType, EdgeType>, Canonizable, Frequented {
  ...
  public ArrayList<Integer> getSortedFreqLabels() { return sortedFreqLabels; }
  ...
  public Graph getSingleGraph() { return singleGraph; }
   ...
   */
  @Override
  public SearchLatticeNode<NodeType, EdgeType>
  extend(final Extension<NodeType, EdgeType> extension) {
    assert extension instanceof GSpanExtension
        : "DFSCode.extend(..) is just applicable for GSpanExtensions";
    final GSpanExtension<NodeType, EdgeType> ext =
        (GSpanExtension<NodeType, EdgeType>)extension;
    ...
    for (GSpanEdge<NodeType, EdgeType> ack = first.next; ack != null;
         ack = ack.next) {
    ...
    return new DFSCode<NodeType, EdgeType>(sortedFreqLabels, singleGraph,
        ...
        .set(ext.getFragment().getHPlistGraph(), nextFirst, nextLast,
             nextParents);
  }
  ...
  private int getNumOfMaxDistinctLabel() {
    ...
    for (Iterator<Entry<Integer, Integer>> iterator =
             labelCounter.entrySet().iterator();
         iterator.hasNext();) {
  ...
  public final Frequency frequency() {
    ...
    if (finalFrequency != null)
      ...
      // PRUNER
      //******* 3
      if (Settings.numLabelAppears > -1 &&
          getNumOfMaxDistinctLabel() > Settings.numLabelAppears) {
        finalFrequency = new IntFrequency(0);
        return finalFrequency;
      }
      ...
      //			{
      //				finalFrequency=new
      //IntFrequency(singleGraph.getFreqThreshold()+1);
      ////				finalFrequency=new IntFrequency(0);
      //				return finalFrequency;
      //			}
      ...
      if (nonCandidates == null) {
        ...
        if (Settings.CACHING) {
          ...
          Decomposer<NodeType, EdgeType> com =
              new Decomposer<NodeType, EdgeType>(me);
          com.decompose();
          ArrayList<HashMap<HPListGraph<NodeType, EdgeType>,
                            ArrayList<Integer>>> maps = com.getMappings();
          for (int i = 0; i < maps.size(); i++) // iterate over edges removed!!
          {
            HashMap<HPListGraph<NodeType, EdgeType>, ArrayList<Integer>>
                edgeRemoved = maps.get(i);

            for (Iterator<Entry<HPListGraph<NodeType, EdgeType>,
                                ArrayList<Integer>>> iterator =
                     edgeRemoved.entrySet().iterator();
                 iterator.hasNext();) {
              Entry<HPListGraph<NodeType, EdgeType>, ArrayList<Integer>>
                  removedEdgeEntry = iterator.next();
              HPListGraph<NodeType, EdgeType> listGraph =
                  removedEdgeEntry.getKey();
              ...
              if (DfscodesCache.cache.containsKey(key)) {
                ...
                for (int j = 0; j < listGraph.getNodeCount(); j++) {
                  HashSet<Integer> nonCans =
                      nonCandidates.get(graphMappings.get(j));
        ...
        ConstraintGraph cg = new ConstraintGraph(
            singleGraph, q, (HashMap<Integer, HashSet<Integer>>)nonCandidates);
        DFSSearch df =
            new DFSSearch(cg, singleGraph.getFreqThreshold(), nonCandidates);
        df.searchExistances();
        ...
        if (Settings.CACHING) {
          ...
          if (nonCands == null)
            System.out.println("nonvands null");
  ...
  public Variable[] getCurrentVariables() { return currentVariables; }

  /*
   * generates a single connected list of possible unused extensions of the
   * nodeA/gNodeA
   */
  private final MinExtension<NodeType, EdgeType>
  getExtensions(final int nodeA, final HPGraph<NodeType, EdgeType> graph,
                final int gNodeA, final int[] usedEdges,
                final int[] usedNodes) {
    ...
    for (int i = graph.getDegree(gNodeA) - 1; i >= 0; --i) {
      ...
      if (usedEdges[edge] == UNUSED) {
        ...
        final MinExtension<NodeType, EdgeType> next =
            new MinExtension<NodeType, EdgeType>().set(
                nodeA, usedNodes[gNodeB], sortedFreqLabels.indexOf(thelabelA),
                edgeLabel, sortedFreqLabels.indexOf(thelabelB),
                graph.getDirection(edge, gNodeA), edge, gNodeB, thelabelA,
                thelabelB);
        next.next = last;
        last = next;
      }
  ...
  public final GSpanEdge<NodeType, EdgeType> getFirst() { return first; }

  /** @return the last GSpanEdge of this DFSCode */
  public final GSpanEdge<NodeType, EdgeType> getLast() { return last; }
  ...
  public GSpanEdge<NodeType, EdgeType> getParent(final int node) {
    if (parents == null) {
      ...
      parents.set(0, first);
      for (GSpanEdge<NodeType, EdgeType> ack = first; ack != null;
           ack = ack.next) {
        if (ack.isForward()) {
  ...
  private boolean isCan(final MinExtensionSet<NodeType, EdgeType> set,
                        final int[] ackNodes, final int[] usedNodes,
                        final int[] usedEdges) {
    ...
    for (int node = hp.getMaxNodeIndex() - 1; node >= 0; --node) {
      ...
      final int nodeLabelIndex =
          sortedFreqLabels.indexOf((hp.getNodeLabel(node)));
  ...
  private boolean isCan2(final GSpanEdge<NodeType, EdgeType> ackEdge,
                         ...
                         final int lastNode, final int[] usedNodes,
                         final int[] usedEdges, final int[] ackNodes,
    ...
    for (MinExtension<NodeType, EdgeType> ack = first;
         ack.compareTo(first) == 0; ack = ack.forward) {
      ...
      if (usedEdges[ack.gEdgei] != UNUSED) {
        ...
        if (!isCan2(ackEdge, set, lastNode, usedNodes, usedEdges, ackNodes,
                    graph)) {
        ...
        if (ack.getNodeB() == UNUSED) { // forward edge
          ...
          final MinExtension<NodeType, EdgeType> exts = getExtensions(
              lastNode + 1, graph, ack.gNodeBi, usedEdges, usedNodes);
          set.addAll(exts);
          // recursiv search
          if (!isCan2(ackEdge.next, set, lastNode + 1, usedNodes, usedEdges,
                      ackNodes, graph)) {
            // smaller DFSCode found
            set.removeAndFreeAll(exts);
            set.relink(ack, next);
          ...
          usedEdges[ack.gEdgei] = 1;
          // recursiv search
          if (!isCan2(ackEdge.next, set, lastNode, usedNodes, usedEdges,
                      ackNodes, graph)) {
            // smaller DFSCode found
            set.relink(ack, next);
            return false;
  ...
  public boolean isCanonical() {
    ...
    final MinExtensionSet<NodeType, EdgeType> set =
        new MinExtensionSet<NodeType, EdgeType>();
  ...
  private int[] getIntArray(final int length) { return new int[length]; }
  /*
   * (non-Javadoc)
   *
   ...
   */
  public DFSCode<NodeType, EdgeType>
  set(final HPListGraph<NodeType, EdgeType> me,
  ...
  public HPListGraph<NodeType, EdgeType> getHPlistGraph() { return me; }
