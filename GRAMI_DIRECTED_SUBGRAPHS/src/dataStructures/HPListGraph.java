 ...
 *            .equals(..))
 */
public class HPListGraph<NodeType, EdgeType>
    implements HPMutableGraph<NodeType, EdgeType> {
  ...
  private static final int NODEA = 0, NODEB = 1, DIRECTION = 2, NEXTEMPTY = 0,
                           EMPTYNODE = 0, EMPTYEDGE = 1, NODECOUNT = 2,
                           EDGECOUNT = 3, MAXNODE = 4, MAXEDGE = 5, DEGREE = 0,
                           INDEGREE = 1, OUTDEGREE = 2, FIRSTEDGE = 3,
                           DEFAULTSIZE = 4, DEFAULTNODESIZE = 6;
   ...
   * @param name
   */
  public HPListGraph(final String name) { this(name, ID++); }
  ...
  @SuppressWarnings("unchecked")
  private final int _addEdge(final int na, final int nb, final EdgeType label,
                             int direction) {
  ...
  private final int _addNode(final NodeType label) {
    ...
    if (idx < 0) { // no empty nodes available
      ...
      if (idx >= length) { // resize nodes
        final int newlength = (int)(RESIZE_SCALE * idx);
        System.arraycopy(node_edges, 0, node_edges = new int[newlength][], 0,
                         length);
      ...
      node_edges[idx][DEGREE] = node_edges[idx][INDEGREE] =
          node_edges[idx][OUTDEGREE] = 0;
    }
    ++status[NODECOUNT];
    node_labels[idx] = label;
    nodesChanged();
    assert node_edges[0] != null
        : dumpGraph("_addNode(" + label.toString() + ") " + idx);
    return idx;
  }
  ...
  public int isFrequent(int embedding) { return isFrequentNodes[embedding]; }
   ...
   *      null, int)
   */
  public int addNodeAndEdgeIndex(final int nodeAIdx, final NodeType nodeLabel,
                                 final EdgeType edgeLabel,
                                 final int direction) {
   ...
   */
  public int addNodeIndex(final NodeType label) { return _addNode(label); }
  ...
  public HPGraph<NodeType, EdgeType> clone() {
    ...
    assert ret.node_edges.length == ns
        : ret.dumpGraph("clone(): node_edges.length wrong " +
  ...
  public IntIterator edgeIndexIterator() {
    return new IntIterator() {
      ...
      public boolean hasNext() {
        ...
        for (next = last + 1; next < max && edge_labels[next] == DELETED;
             ++next) {
      ...
      public void remove() { _deleteEdge(last); };
    };
  }

  private final void edgesChanged() { edges = null; }
   ...
   */
  public int getEdgeCount() { return status[EDGECOUNT]; }
  ...
  public IntIterator getEdgeIndices(final int nodeIdx) {
    return new IntIterator() {
      ...
      public boolean hasNext() { return pos < node[DEGREE] + FIRSTEDGE; }
  ...
  public IntIterator getInEdgeIndices(final int nodeIdx) {
    return new IntIterator() {
      ...
      public boolean hasNext() {
        ...
        for (next = last + 1;
             next < node[DEGREE] + FIRSTEDGE &&
             getDirection(node[next], nodeIdx) != Edge.INCOMING;
             ++next) {
   ...
   */
  public int getMaxEdgeIndex() { return status[MAXEDGE]; }
   ...
   */
  public int getMaxNodeIndex() { return status[MAXNODE]; }
   ...
   */
  public int getNodeCount() { return status[NODECOUNT]; }
  ...
  public IntIterator getOutEdgeIndices(final int nodeIdx) {
    return new IntIterator() {
      ...
      public boolean hasNext() {
        ...
        for (next = last + 1;
             next < node[DEGREE] + FIRSTEDGE &&
             getDirection(node[next], nodeIdx) != Edge.OUTGOING;
             ++next) {
  ...
  public boolean isValidEdge(final int edgeIdx) {
    return !(edgeIdx < 0 || edgeIdx >= status[MAXEDGE] ||
             edge_labels[edgeIdx] == DELETED);
  }

  public boolean isValidNode(final int nodeIdx) {
    return !(nodeIdx < 0 || nodeIdx >= status[MAXNODE] ||
             node_labels[nodeIdx] == DELETED);
  }
  ...
  public IntIterator nodeIndexIterator() {
    return new IntIterator() {
      ...
      public boolean hasNext() {
        ...
        for (next = last + 1; next < max && node_labels[next] == DELETED;
             ++next) {
      ...
      public void remove() { _deleteNode(last); };
    };
  }

  private final void nodesChanged() { nodes = null; }
  ...
  public String toString() { return DFScodeSerializer.serialize(this); }
}
