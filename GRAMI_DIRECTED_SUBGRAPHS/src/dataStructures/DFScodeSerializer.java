...
public class DFScodeSerializer {
  ...
  public static String serialize(final HPListGraph graph) {
    ...
    for (int nodeIdx = nodes.nextSetBit(0); nodeIdx >= 0;
         nodeIdx = nodes.nextSetBit(nodeIdx + 1)) {
      int label = (Integer)graph.getNodeLabel(nodeIdx);
      text += "v " + nodeIdx + " " + label + "\n";
    }

    for (int edgeIdx = edges.nextSetBit(0); edgeIdx >= 0;
         edgeIdx = edges.nextSetBit(edgeIdx + 1)) {
