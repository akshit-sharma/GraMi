...
public class myNode {
  ...
  private HashMap<Integer, ArrayList<MyPair<Integer, Double>>>
      reachableNodes; // represented by Label~<nodeID,edge_label>, represents
                      // the outgoing nodes

  private HashMap<Integer, ArrayList<MyPair<Integer, Double>>>
      reachedBYNodes; // represented by Label~<nodeID,edge_label>, represents
                      // the ingoing nodes
  ...
  public void addreachableNode(myNode node, double edgeLabel) {
    if (reachableNodes == null)
      reachableNodes =
          new HashMap<Integer, ArrayList<MyPair<Integer, Double>>>();

    ArrayList<MyPair<Integer, Double>> list =
        reachableNodes.get(node.getLabel());
    if (list == null) {
  ...
  private void addreachedBYNodes(myNode node, double edgeLabel) {
    if (reachedBYNodes == null)
      reachedBYNodes =
          new HashMap<Integer, ArrayList<MyPair<Integer, Double>>>();
    ArrayList<MyPair<Integer, Double>> list =
        reachedBYNodes.get(node.getLabel());
    if (list == null) {
      list = new ArrayList<MyPair<Integer, Double>>();
  ...
  public void printOutReachableNodes() {
    if (reachableNodes == null)
      return;
    for (Iterator<ArrayList<MyPair<Integer, Double>>> iterator =
             reachableNodes.values().iterator();
         iterator.hasNext();) {
      ArrayList<MyPair<Integer, Double>> arr = iterator.next();
      for (int i = 0; i < arr.size(); i++) {
        System.out.println("Node: " + ID + " is within reach of Node " +
                           arr.get(i));
      }
    }
  }
  ...
  public void
  setReachableNodes(DijkstraEngine dj,
                    HashMap<Integer, HashMap<Integer, myNode>> freqNodesByLabel,
                    HPListGraph graph) {
    for (Iterator<java.util.Map.Entry<Integer, HashMap<Integer, myNode>>> it =
      ...
      for (Iterator<myNode> iterator = tmp.values().iterator();
           iterator.hasNext();) {
  ...
  public void setReachableNodes_1hop(
    ...
    for (; it.hasNext();) {
      int edge = it.next();
      myNode otherNode =
          graph.getNode(graph.getListGraph().getOtherNode(edge, getID()));
      if (freqNodesByLabel.containsKey(otherNode.getLabel()))
        addreachableNode(otherNode, graph.getListGraph().getEdgeLabel(
                                        getID(), otherNode.getID()));
    }
  }
  ...
  public ArrayList<MyPair<Integer, Double>>
  getRechableWithNodeIDs(int label, double edgeLabel) {
    if (reachableNodes == null)
      return new ArrayList<MyPair<Integer, Double>>();

    ArrayList<MyPair<Integer, Double>> tempArr =
        new ArrayList<MyPair<Integer, Double>>();
  ...
  public HashMap<Integer, ArrayList<MyPair<Integer, Double>>>
  getReachableWithNodes() {
  ...
  public ArrayList<MyPair<Integer, Double>>
  getRechableByNodeIDs(int label, double edgeLabel) {
    if (reachedBYNodes == null)
      return new ArrayList<MyPair<Integer, Double>>();
