...
public class Util {
  ...
  public static ArrayList<MyPair<Integer, Double>>
  getIntersection(ArrayList<ArrayList<MyPair<Integer, Double>>> sets) {
    ArrayList<MyPair<Integer, Double>> inter =
  ...
  public static ArrayList<Integer> getIntersection(ArrayList<Integer> set1,
  ...
  getZerosIntersectionIndices(ArrayList<VariableCandidates> sets) {
    ArrayList<Point> points = new ArrayList<Point>();

    for (int i = 0; i < sets.size(); i++) {
      VariableCandidates firstVariableCandidate = sets.get(i);
      ArrayList<MyPair<Integer, Double>> firstElement =
          firstVariableCandidate.getCandidates();
      if (firstElement == null)
        continue;
      for (int j = i + 1; j < sets.size(); j++) {
        VariableCandidates secondVariableCandidate = sets.get(j);
        ArrayList<MyPair<Integer, Double>> secondElement =
            secondVariableCandidate.getCandidates();
        if (secondElement == null)
          continue;
        boolean doesIntersect = false;
        for (int k = 0; k < firstElement.size(); k++) {
          if (MyPair.getIndexOf(secondElement, firstElement.get(k).getA()) !=
              -1) {
            doesIntersect = true;
            break;
          }
        ...
        if (doesIntersect == false)
          points.add(new Point(firstVariableCandidate.getVariableID(),
                               secondVariableCandidate.getVariableID()));
      }
    }
    return points;
  }
  ...
  clone(HashMap<Integer, HashSet<Integer>> in) {
    HashMap<Integer, HashSet<Integer>> out =
