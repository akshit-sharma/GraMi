...
public class DFSSearch {
  ...
  public DFSSearch(ConstraintGraph cg, int minFreqThreshold,
    ...
    for (int i = 0; i < variables.length; i++) {
      ...
      result[i] = new Variable(variables[i].getID(), variables[i].getLabel(),
                               list, variables[i].getDistanceConstrainedWith(),
  ...
  private void AC_3_New(Variable[] input, int freqThreshold) {
    ...
    for (int i = 0; i < input.length; i++) {
      ...
      ArrayList<MyPair<Integer, Double>> list =
          currentVar.getDistanceConstrainedWith();
  ...
  private void refine_Newest(Variable v1, Variable v2, double edgeLabel,
    ...
    for (Iterator<myNode> iterator = listA.values().iterator();
         iterator.hasNext();) {
      myNode n1 = iterator.next(); // get the current node
      if (n1.hasReachableNodes() ==
          false) // prune a node without reachable nodes
        continue;

      ArrayList<MyPair<Integer, Double>> neighbors = n1.getRechableWithNodeIDs(
          labelB, edgeLabel); // get a list of current node's neighbors
      if (neighbors == null)
        continue;
      ...
      {
        MyPair<Integer, Double> mp =
            iterator2.next(); // get current neighbor details
        // check the second column if it contains the current neighbor node
        if (listB.containsKey(mp.getA())) {
          // if true, put the current node in the first column, and the neighbor
          // node in the second column
  ...
  public int
  hasBeenPrecomputed(Variable[] autos, int[] preComputed,
                     int index) // if returns same index should search in it!!
  {
    HashMap<Integer, myNode> list = autos[index].getList();

    for (Iterator<Integer> iterator = list.keySet().iterator();
         iterator.hasNext();) {
  ...
  /**
   * given the graph, check whther it is Acyclic or not (we assume the graph is
   * connected)
   * @param me
   * @return
   */
  public static boolean isItAcyclic(HPListGraph<Integer, Double> me) {
    ...
    while (visited.size() < me.getNodeCount()) {
      ...
      // all edges!
      for (final IntIterator eit = me.getEdgeIndices(currentNodeID);
           eit.hasNext();) {
        int edgeID = eit.next();
        int nID = me.getNodeA(edgeID);
        if (nID == currentNodeID)
          nID = me.getNodeB(edgeID);
  ...
  public void searchExistances() {
    ...
    if (variables.length == 2 &&
        variables[0].getLabel() != variables[1].getLabel()) {
    ...
    if (Settings.DISTINCTLABELS && areAllLabelsDistinct(qry.getListGraph()) &&
    ...
    for (int i = variables.length - 1; i >= 0; i--) {
      ...
      if (Settings.isAutomorphismOn && atm.hasAutomorphisms()) {
        ...
        if (i != preIndex) {
          ...
          variables[i].setList(
              (HashMap<Integer, myNode>)variables[preIndex].getList().clone());
          result[i].setList(
              (HashMap<Integer, myNode>)result[preIndex].getList().clone());
        }
      }
      if (search == true) {
        // fast check
        ...
        tmp.clear();
        for (Iterator<myNode> iterator = firstList.values().iterator();
             iterator.hasNext();) {
          ...
          if (Settings.isApproximate) {
            ...
            for (int k = 0; k < variables.length; k++) {
              int listSize =
                  (int)(variables[k].getList().size() * weight.doubleValue());
            ...
            finalWeight = finalWeight.multiply(
                new BigInteger((variables.length * 2) + ""));
            finalWeight = finalWeight.add(
                new BigDecimal(Settings.approxConstant).toBigInteger());
          }
          ...
          if (value == -1) {
            ...
            for (int j = 0; j < variables.length; j++) {
              ...
              if (Settings.isAutomorphismOn && atm.hasAutomorphisms()) {
                ...
                for (Iterator<Integer> iterator2 = list.keySet().iterator();
                     iterator2.hasNext();) {
                  int nodeIndex = iterator2.next();
                  if (!result[nodeIndex].getList().containsKey(
                          assignedNode.getID())) {
                    result[nodeIndex].getList().put(assignedNode.getID(),
                                                    assignedNode);
                  }
                }
              } else {
            ...
            // check if the size of the list has passed already the
            // minFreqThreshold!!
            ...
            } else // TRULY SEARCH INTO IT!!
            {
              ...
              for (int j = 0; j < tmp.size(); j++) {
                ...
                if (Settings.isDecomposeOn == true) // decomposition is ON !!!
                {
                  ...
                  HPListGraph<Integer, Double> actualPatternGraph =
                      qry.getListGraph();
                  Decomposer<Integer, Double> com =
                      new Decomposer<Integer, Double>(actualPatternGraph);
                  com.decompose();
                  ArrayList<HashMap<HPListGraph<Integer, Double>,
                                    ArrayList<Integer>>> maps =
                      com.getMappings();
                  int counter = 0;
                  for (int k = 0; k < maps.size();
                       k++) // iterate over edges removed!!
                  {
                    HashMap<HPListGraph<Integer, Double>, ArrayList<Integer>>
                        edgeRemoved = maps.get(k);

                    for (Iterator<Entry<HPListGraph<Integer, Double>,
                      ...
                      ArrayList<Integer> graphMappings =
                          removedEdgeEntry
                              .getValue(); // pattern nodeID ~ original ID
                      ...
                      Query qry =
                          new Query((HPListGraph<Integer, Double>)listGraph);
                      ...
                      isExistant = df.searchParticularExistance(
                          instance, correspondingINdex);
                      if (isExistant == false) {
                        firstList.remove(firstNode.getID());
                        HashSet<Integer> nonCan =
                            nonCandidates.get(firstVB.getID());
                        if (nonCan == null) {
                          nonCan = new HashSet<Integer>();
                          nonCan.add(firstNode.getID());
                          nonCandidates.put(firstVB.getID(), nonCan);
                ...
                if (value == -1) {
                  ...
                  for (int k = 0; k < variables.length; k++) {
                    ...
                    if (Settings.isAutomorphismOn && atm.hasAutomorphisms()) {
                      ...
                      for (Iterator<Integer> iterator2 =
                      ...
                      if (!result[k].getList().containsKey(
                              assignedNode.getID())) {
                        result[k].getList().put(assignedNode.getID(),
                                                assignedNode);
                      }
                    }
                  }
  ...
  public boolean searchParticularExistance(AssignmentInstance instance,
                                           int orderINdex) {
    sOrder.reset();
  ...
  private int searchExistances(AssignmentInstance instance) {
    ...
    if (index != -1) {
      ...
      ArrayList<VariableCandidates> variableCandidates =
          new ArrayList<VariableCandidates>();
      ...
      for (int i = 0; i < constrainingVariables.size(); i++) {
        ...
        if (cnVariableInstance != null) {
          candidates.add(cnVariableInstance.getRechableByNodeIDs(
              currentVB.getLabel(), edgeLabel));
          variableCandidates.add(new VariableCandidates(
              cnVariableIndex, cnVariableInstance.getRechableByNodeIDs(
                                   currentVB.getLabel(), edgeLabel)));
        }
      }
      ...
      for (int i = 0; i < constrainingBYVariables.size(); i++) {
        ...
        myNode cnVariableInstance = instance.getAssignment(cnVariableIndex);
        if (cnVariableInstance != null) {
          candidates.add(cnVariableInstance.getRechableWithNodeIDs(
              currentVB.getLabel(), edgeLabel));
          variableCandidates.add(new VariableCandidates(
              cnVariableIndex, cnVariableInstance.getRechableWithNodeIDs(
                                   currentVB.getLabel(), edgeLabel)));
        }
      }

      ArrayList<MyPair<Integer, Double>> finalCandidates =
          ...
          if (AssignmentInstance.ensureIDValidty(instance)) {
            ...
            if (hasResult == -1)
              ...
              if (currentVB.getID() != hasResult) {
                ...
                if (Settings.isApproximate)
                  numberOfIterations =
                      numberOfIterations.add(new BigInteger("1"));
                instance.deAssign(currentVB.getID());
                return hasResult;
              }
            } else
    ...
    } else // index ==-1 means that I reached the point where the assignment is
           // legal
    {
  ...
  public void stopSearching() { isStopped = true; }

  //***************************************
  public int getResultCounter() { return resultCounter; }
  ...
  private void searchOrder(Variable vb) {
    ...
    ArrayList<MyPair<Integer, Double>> constrains =
        vb.getDistanceConstrainedWith();
    ...
    ArrayList<MyPair<Integer, Double>> constrainsBY =
        vb.getDistanceConstrainedBy();
  ...
  public Variable[] getResultVariables() { return result; }
  ...
  private Variable[] cloneDomian(Variable[] domain) {
    ...
    for (int i = 0; i < cloneDomian.length; i++) {
      ...
      cloneDomian[i] = new Variable(
          currentDomain.getID(), currentDomain.getLabel(),
          (HashMap<Integer, myNode>)currentDomain.getList().clone(), null,
          null);
    }

    // add constraints !!
    for (int i = 0; i < cloneDomian.length; i++) {
      cloneDomian[i].setDistanceConstrainedBy(
          domain[i].getDistanceConstrainedBy());
      cloneDomian[i].setDistanceConstrainedWith(
          domain[i].getDistanceConstrainedWith());
    }
    return cloneDomian;
  }

  private Variable[] look_ahead(int index, myNode node,
  ...
  private int solve(ArrayList<Integer> X, Variable[] currentDomain,
    ...
    if (index != -1) {
      ...
      if (!isAnyEmptyD_dash) {
        ...
        for (Iterator<Entry<Integer, myNode>> iterator =
          ...
          Variable[] D_dash_dash =
              look_ahead(index, nodeEntry.getValue(), D_dash);
    ...
    for (int i = 0; i < domain.length; i++) {
      ...
      if (currentDomain.getList().size() == 1) {
        ...
        for (int j = 0; j < consBY.size(); j++) {
          ...
          ArrayList<MyPair<Integer, Double>> candNodes =
              nodereachBy.get(vb.getLabel());
        ...
        ArrayList<MyPair<Integer, Double>> consWith =
            domain[i].getDistanceConstrainedWith();
  ...
  public void searchAll() {
    setVariableVisitingOrder(
        getMaxDegreeVariableIndex()); // set variable visit order
    int index = -1;
