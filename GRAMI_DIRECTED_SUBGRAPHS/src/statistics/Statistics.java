...
public class Statistics<NodeType, EdgeType> {
  ...
  private void computeStats() {
    ...
    // Local
    HashMap<Integer, Integer> distinctLabelsSize =
        new HashMap<Integer, Integer>();
    HashMap<Integer, Point> distinctLabelsMaxNode =
        new HashMap<Integer, Point>();
    HashMap<Integer, Integer> distinctLabelsMaxNodeIndex =
        new HashMap<Integer, Integer>();
    HashMap<Integer, Point> distinctLabelsMaxEdge =
        new HashMap<Integer, Point>();
    HashMap<Integer, Integer> distinctLabelsMaxEdgeIndex =
        new HashMap<Integer, Integer>();
    for (int i = 0; i < result.size(); i++) {
      ...
      if (localSize == null) {
        ...
        distinctLabelsMaxNode.put(numOfDistinct,
                                  new Point(nodeCount, edgeCount));
        distinctLabelsMaxNodeIndex.put(numOfDistinct, i);
        distinctLabelsMaxEdge.put(numOfDistinct,
                                  new Point(nodeCount, edgeCount));
        ...
        Point previousMaxNode =
            distinctLabelsMaxNode.get(numOfDistinct); // compare with x
        Point previousMaxEdge =
            distinctLabelsMaxEdge.get(numOfDistinct); // compare with y
    ...
    for (Iterator<Integer> iterator = distinctLabelsSize.keySet().iterator();
      ...
      DistinctLabelStat stat =
          new DistinctLabelStat(distinctSize, localMaxNode, localMaxEdge,
                                indexlocalMaxNode, indexlocalMaxEdge);
  ...
  public String toString2() {
    ...
    for (Iterator<Integer> iterator = distinctLabels.keySet().iterator();
         iterator.hasNext();) {
      Integer numOfDistinct = iterator.next();
