...
public class SPpruner {
  ...
  public void
  getPrunedLists(ArrayList<HashMap<Integer, myNode>> candidatesByNodeID,
                 Query qry) {
    ...
    for (int i = 0; i < qry.getListGraph().getNodeCount(); i++) {
      ...
      variables[i] =
          new Variable(i, label, candidatesByNodeID.get(i), null, null);
    }
  ...
  getPrunedLists(HashMap<Integer, HashMap<Integer, myNode>> nodesByLabel,
    ...
    HashMap<Integer, HashMap<Integer, Integer>> nodeOutLabelDegrees =
        new HashMap<Integer,
                    HashMap<Integer, Integer>>(); // nodeID-->(Label,Degree)
    ...
    for (int i = 0; i < qry.getListGraph().getNodeCount(); i++) {
      ...
      for (Iterator<Entry<Integer, myNode>> it =
        ...
        if (degreeOutCons != null)
          for (Iterator<Entry<Integer, Integer>> iterator =
                   degreeOutCons.entrySet().iterator();
               iterator.hasNext();) {
        ...
        if (isValidNode && degreeInCons != null) {
          for (Iterator<Entry<Integer, Integer>> iterator =
                   degreeInCons.entrySet().iterator();
               iterator.hasNext();) {
            Entry<Integer, Integer> entry = iterator.next();
            int label = entry.getKey();
            int degree = entry.getValue();
  ...
  public void getPrunedLists(Graph graph, Query qry,
    ...
    for (int i = 0; i < qry.getListGraph().getNodeCount(); i++) {
      int label = qry.getListGraph().getNodeLabel(i);
      pruned.put(i, (HashMap<Integer, myNode>)graph.getFreqNodesByLabel()
                        .get(label)
                        .clone());
    }

    for (Iterator<Entry<Integer, HashSet<Integer>>> iterator =
             nonCandidates.entrySet().iterator();
         iterator.hasNext();) {
  ...
  public static void
  printSPs(HashMap<Integer, HashMap<Integer, myNode>> pruned) {
