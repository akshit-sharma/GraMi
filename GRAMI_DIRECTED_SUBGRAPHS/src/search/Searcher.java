...
public class Searcher<NodeType, EdgeType> {
  ...
  public Searcher(String path, int freqThreshold, int shortestDistance)
      throws Exception {
  ...
  public void initialize() {
    ...
    HashMap<Integer, HashMap<Integer, myNode>> freqNodesByLabel =
        singleGraph.getFreqNodesByLabel();
    ...
    for (Iterator<java.util.Map.Entry<Integer, HashMap<Integer, myNode>>> it =
      ...
      for (Iterator<myNode> iterator = tmp.values().iterator();
        ...
        if (neighbours != null)
          for (Iterator<Integer> iter = neighbours.keySet().iterator();
               iter.hasNext();) {
            ...
            for (Iterator<MyPair<Integer, Double>> iter1 =
                     neighbours.get(secondLabel).iterator();
                 iter1.hasNext();) {
              ...
              final GSpanEdge<NodeType, EdgeType> gedge =
                  new GSpanEdge<NodeType, EdgeType>().set(
                      0, 1, labelA, (int)edgeLabel, labelB, 1, firstLabel,
                      secondLabel);
              ...
              if (!initials.containsKey(gedge)) {
                ...
                HPListGraph<NodeType, EdgeType> lg =
                    new HPListGraph<NodeType, EdgeType>();
                gedge.addTo(lg);
                DFSCode<NodeType, EdgeType> code =
                    new DFSCode<NodeType, EdgeType>(sortedFrequentLabels,
                                                    singleGraph, null)
    ...
    for (final Iterator<Map.Entry<GSpanEdge<NodeType, EdgeType>,
                                  DFSCode<NodeType, EdgeType>>> eit =
  ...
  public void search() {
    ...
    RecursiveStrategy<NodeType, EdgeType> rs =
        new RecursiveStrategy<NodeType, EdgeType>();
    result = (ArrayList<HPListGraph<NodeType, EdgeType>>)rs.search(
        algo, this.freqThreshold.intValue());
  }
