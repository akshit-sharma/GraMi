...
public class Algorithm<NodeType, EdgeType>
    implements AlgorithmInterface.Algorithm<NodeType, EdgeType>,
  ...
  private class MyIterator
      implements Iterator<SearchLatticeNode<NodeType, EdgeType>> {
    final Iterator<Map.Entry<GSpanEdge<NodeType, EdgeType>,
                             DFSCode<NodeType, EdgeType>>> entryit;

    final boolean del;

    Map.Entry<GSpanEdge<NodeType, EdgeType>, DFSCode<NodeType, EdgeType>> last =
        null;

    MyIterator(final Map<GSpanEdge<NodeType, EdgeType>,
                         DFSCode<NodeType, EdgeType>> initials,
               final boolean del) {
      entryit = initials.entrySet().iterator();
      this.del = del;
    }

    public boolean hasNext() { return entryit.hasNext(); }
  ...
  public void setInitials(Map<GSpanEdge<NodeType, EdgeType>,
                              DFSCode<NodeType, EdgeType>> initials) {
  ...
  public Extender<NodeType, EdgeType> getExtender(int minFreq) {
    ...
    final GSpanExtender<NodeType, EdgeType> extender =
        new GSpanExtender<NodeType, EdgeType>();
    ...
    //  ... over generation ...
    curFirst = gen =
        new EmbeddingBasedGenerationStep<NodeType, EdgeType>(curFirst);
    // .. to prefilters

    curFirst = new FrequencyPruningStep<NodeType, EdgeType>(
        curFirst, new IntFrequency(minFreq), null);
    ...
    // YES
    generationFirst =
        new RightMostExtension<NodeType, EdgeType>(generationFirst);
