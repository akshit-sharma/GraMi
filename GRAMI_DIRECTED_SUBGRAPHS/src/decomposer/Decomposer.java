...
public class Decomposer<NodeType, EdgeType> {
  ...
  public ArrayList<HashMap<HPListGraph<NodeType, EdgeType>, ArrayList<Integer>>>
  getMappings() {
    return mappings;
  }
  ...
  public void printResults() {
    for (int i = 0; i < mappings.size(); i++) {
      ...
      for (Iterator<Entry<HPListGraph<NodeType, EdgeType>, ArrayList<Integer>>>
               iterator = currentMap.entrySet().iterator();
           iterator.hasNext();) {
        Entry<HPListGraph<NodeType, EdgeType>, ArrayList<Integer>> entry =
            iterator.next();
  ...
  public void decompose() {
    ...
    for (int i = 0; i < currentGraph.getEdgeCount(); i++) {
      ...
      HashMap<HPListGraph<NodeType, EdgeType>,
              ArrayList<Integer>> currentEdgeMapping =
      ...
      for (int j = 0; j < newGraph.getNodeCount(); j++) {
        ...
        ArrayList<Integer> mapping =
            new ArrayList<Integer>(); // corresponding mapping !!
        ...
        while (!DFSstack.isEmpty()) {
          ...
          for (IntIterator it = newGraph.getEdgeIndices(currentNodeIDmapping);
               it.hasNext();) {
            int edge = (int)it.next();
            int otherNodeMapping =
                newGraph.getOtherNode(edge, currentNodeIDmapping);
            ...
            if (otherNode == -1) {
              ...
              connectedComp.addNodeIndex(
                  newGraph.getNodeLabel(otherNodeMapping));
              mapping.add(otherNodeMapping);
            }

            connectedComp.addEdgeIndex(
                currentNodeID, otherNode, (EdgeType) "1",
                newGraph.getDirection(edge, currentNodeIDmapping));

            DFSstack.push(otherNodeMapping);
          }
        }
        if (connectedComp.getNodeCount() > 1 &&
            mapping.contains(currentGraph.getNodeCount() - 1))
